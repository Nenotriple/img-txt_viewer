...
class BatchImgEdit:
    def __init__(self):
        ...
        self.previous_adjustments = {}  # Stores last used adjustments
        self._var_to_option = {}  # Maps variable names to option names
        self._advanced_vars = {}  # Advanced option variables
        self._advanced_defaults = {}  # Default values for advanced options
        self.original_selected_image = None  # Original image for preview
        self._original_selected_filename = None  # Filename of original selected image
        self._image_preview_update_id = None  # Tkinter after() id for preview update
        self.image_filelist = []  # List of image files in working directory
        self.is_batch_running = False  # Flag for batch processing state
...
#region UI Creation
    ...
    def create_options_frame(self):
        ...
        def add_advanced_option(parent_frame, option_name, param_name, default_value, from_val, to_val, label_text=None):
            """Factory for creating advanced option controls"""
            ...
            ttk.Label(value_frame, textvariable=var, width=5, anchor="e", foreground="gray").pack(side="right", padx=(4, 0))
            # Scale frame
        ...
        def add_scale(name, var_attr, scale_attr, from_=-100, to_=100, pady=10, advanced_options=None):
            ...
            if advanced_options:
                toggle_btn = ttk.Button(top_row, text="▼", width=3, command=lambda: self._toggle_advanced_options(name))
        ...
        option_defs = [
            ...
            ("Highlights", "highlights_var", "highlights_scale", -100, 100, 10, self._create_highlights_advanced),
            ("Shadows", "shadows_var", "shadows_scale", -100, 100, 10, self._create_shadows_advanced),
        ...
        for item in option_defs:
            ...
            add_scale(display_name, var_attr, scale_attr, from_, to_, pady, advanced_options)


    def _toggle_advanced_options(self, option_name):
        """Toggle visibility of advanced options for a specific adjustment"""
    ...
    def _create_highlights_advanced(self, parent_frame, add_option):
        add_option(parent_frame, "Highlights", "threshold", 128, 1, 256)
        add_option(parent_frame, "Highlights", "blur_radius", 0, 0, 20, "Blur Radius:")


    def _create_shadows_advanced(self, parent_frame, add_option):
        add_option(parent_frame, "Shadows", "threshold", 128, 1, 256)
        add_option(parent_frame, "Shadows", "blur_radius", 0, 0, 20, "Blur Radius:")
